...
$router->get('/api/admin/career-club', function () {
    ...
    $members = Database::fetchAll("SELECT id, name, designation, position, photo_path, sort_order FROM career_club ORDER BY sort_order");
...
$router->post('/api/admin/career-club', function () {
    try {
        ...
        $maxOrder = Database::fetch("SELECT MAX(sort_order) as max_order FROM career_club");
...
$router->put('/api/admin/career-club/{id}', function (array $params) {
    try {
        ...
        if (!empty($updateData)) Database::update('career_club', $updateData, 'id = ?', ['id' => $params['id']]);
...
$router->delete('/api/admin/career-club/{id}', function (array $params) {
    try {
        Auth::requireRole('admin');
        $deleted = Database::fetch("SELECT sort_order FROM career_club WHERE id = ?", [$params['id']]);
        if (!$deleted) Response::notFound('Member not found');
        Database::delete('career_club', 'id = ?', [$params['id']]);
        Database::query("UPDATE career_club SET sort_order = sort_order - 1 WHERE sort_order > ?", [$deleted['sort_order']]);
...
$router->post('/api/admin/career-club/{id}/move-up', function (array $params) {
    try {
        ...
        $above = Database::fetch("SELECT id, sort_order FROM career_club WHERE sort_order < ? ORDER BY sort_order DESC LIMIT 1", [$current['sort_order']]);
        if (!$above) Response::success(null, 'Already at top');
        Database::update('career_club', ['sort_order' => $above['sort_order']], 'id = ?', [$current['id']]);
        Database::update('career_club', ['sort_order' => $current['sort_order']], 'id = ?', [$above['id']]);
...
$router->post('/api/admin/career-club/{id}/move-down', function (array $params) {
    try {
        ...
        $current = Database::fetch("SELECT id, sort_order FROM career_club WHERE id = ?", [$params['id']]);
        if (!$current) Response::notFound('Member not found');
        $below = Database::fetch("SELECT id, sort_order FROM career_club WHERE sort_order > ? ORDER BY sort_order ASC LIMIT 1", [$current['sort_order']]);
        if (!$below) Response::success(null, 'Already at bottom');
        Database::update('career_club', ['sort_order' => $below['sort_order']], 'id = ?', [$current['id']]);
        Database::update('career_club', ['sort_order' => $current['sort_order']], 'id = ?', [$below['id']]);
